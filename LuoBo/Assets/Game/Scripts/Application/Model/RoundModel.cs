...
public class RoundModel : Model
{
    ...
    #region 字段
    List<Round> m_Rounds = new List<Round>(); // 该关卡所有出怪信息
    int m_roundIndex = -1; // 当前是第几波怪物

    bool m_allRoundsComplete = false; // 是否所有怪物释放完
    #endregion
    ...
    IEnumerator RunRound()
    {
        for (int i = 0; i < m_Rounds.Count; i++)
        {
            ...
            for (int j = 0; j < round.Count; j++)
            {
                ...
                // 出怪事件
                SpawnMonsterArgs e1 = new SpawnMonsterArgs();
                e1.MonsterType = round.Monster;
                SendEvent(Consts.E_SpawnMonster, e1);
            }
            // 回合间隔时间
